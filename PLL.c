...
void PLL_Init(void){
  ...
  SYSCTL_RCC2_R = (SYSCTL_RCC2_R&~(unsigned long) 0x1FC00000)  // clear system clock divider
                  + (4<<22);      // configure for 50 MHz clock // CHANGE TO 80 MHz CONFIGURATION
...
void us_delay(unsigned long delay){ // calls the systick's 1us delay a given number of times 
	unsigned long j;
	for(j = 0; j < delay; j++) {
  SysTick_Wait(80);
  }
}
void ms_delay(unsigned long delay){
	unsigned long j;
	for(j = 0; j < delay; j++) {
  SysTick_Wait(80000);  //calls systick's 1ms delay a given number of times
  }
}
